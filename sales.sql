...
-- 3. (Optional but recommended) Index for Filtering/Grouping by Date
...
--8.  Find Sales Transactions that are NOT linked to a valid Product (Orphaned Records)
SELECT
    ft.Order_ID,
    ft.product_id
FROM
...
LEFT JOIN
    product_table AS dp ON ft.product_id = dp.product_id
WHERE
    dp.product_id IS NULL;-- (If this returns zero rows, your data is clean!)
